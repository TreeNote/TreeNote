...
id_index_dict = dict()  # New indexes are created by TreeModel.index(). That function stores the index in this dict.
db_name = 'tree'
...
class Updater(QThread):
    ...
    def run(self):  # this updates the view
        all_changes = db.changes(descending=True)
        last_seq = all_changes['results'][0]['seq']
        changes_list = db.changes(feed='continuous', heartbeat=sys.maxsize, include_docs=True, since=last_seq)  # no need for heartbeet, because the db is local
        for line in changes_list:
            print(line)
            if 'doc' in line and 'deleted' not in line:
                db_item = line['doc']
                item_id = db_item['_id']
                if item_id in id_index_dict:  # update the view only if the item is already loaded
                    with self.tree_model.lock:
                        ...
                        def moved_vertical():
                            ...
                            if up_or_down == -1:
                                ...
                                self.tree_model.index(position + count - 1, 0, index)  # calling index() refreshes the id_index_dict of that item
                            elif up_or_down == +1:
                                item.childItems.insert(position, item.childItems.pop(position + count))
                                self.tree_model.index(position, 0, index)  # calling index() refreshes the id_index_dict of that item
                            for i in range(count):
                                index_moved_item = self.tree_model.index(position + up_or_down + i, 0, index)  # calling index() refreshes the id_index_dict of that item
                                if i == 0:
...
class Tree_item(object):
    ...
    def deserialise_children_from_db(self):
        if self.childItems is None:  # child_count get's called often. Improve performance by deserialising only once
            self.childItems = []
            children_id_list = db[self.id]['children'].split()
            self.insert_children(0, len(children_id_list))
            for i in range(len(children_id_list)):
                self.childItems[i].text = db[children_id_list[i]]['text']
                self.childItems[i].id = children_id_list[i]
...
class TreeModel(QAbstractItemModel):
    ...
    def index(self, row, column, parent=QModelIndex()):
        ...
        if childItem:
            ...
            id_index_dict[childItem.id] = index
            return index
        else:
            return QModelIndex()
    ...
    def rowCount(self, parent=QModelIndex()):
        parentItem = self.getItem(parent)
        parentItem.deserialise_children_from_db()
    ...
    def move_left(self, indexes):
        ...
        if parent_parent_item is not None:  # stop moving left if parent is root_item
            ...
            parent_parent_item_index = id_index_dict[parent_parent_item.id]
